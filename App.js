...
export default function App() {
  ...
  const dashboardStatus = useMemo(() => {
    ...
    return currentMonthClaims.every(d => d.completed) ? 'completed' : 'pending';
  }, [claims]);
  ...
  const yearlyStats = useMemo(() => {
    ...
    return Object.keys(summary).map(name => ({
      name, sum: summary[name], percent: totalYearSum > 0 ? parseFloat((summary[name] / totalYearSum * 100).toFixed(1)) : 0
    ...
    <View style={[styles.container, { backgroundColor: theme.bg }]}>
      ...
      <View style={styles.contentArea}>
        ...
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={{ flex: 1 }} keyboardVerticalOffset={Platform.OS === "ios" ? 20 : 0}>
          ...
          {activeTab === 'list' && (
            <View style={{ flex: 1 }}>
              <View style={styles.searchContainer}><TextInput style={[styles.searchInput, { backgroundColor: theme.inputBg, color: theme.text }]} placeholder="Suchen..." value={searchQuery} onChangeText={setSearchQuery} placeholderTextColor={theme.subText}/></View>
              <ScrollView>
                    ...
                    {filteredIncomes.map(item => (
                      <View key={item.id} style={[styles.card, { backgroundColor: theme.card }]}>
                        <TouchableOpacity style={styles.cardMain} onLongPress={() => deleteIncome(item.id)}>
                          ...
                          <View style={{ flex: 1 }}><Text style={[styles.cardName, { color: theme.text }]}>{item.name}</Text><Text style={[styles.cardSub, { color: theme.subText }]}>{item.dates[0]?.dateString}</Text></View>
                    ...
                    return (
                      <View key={item.id} style={[styles.card, { backgroundColor: theme.card }]}>
                        <TouchableOpacity activeOpacity={0.8} style={styles.cardMain} onPress={() => setExpandedId(isExpanded ? null : item.id)} onLongPress={() => deleteWholeClaim(item.id)}>
                          <View style={[styles.cardIconBox, { backgroundColor: isDarkMode ? '#2C2C2E' : '#F0F4FF' }, overdue && {backgroundColor: isDarkMode ? '#3D1B1B' : '#FFF0F0'}]}><Text style={{fontSize: 20}}>{overdue ? '⚠️' : '📅'}</Text></View>
                          <View style={{ flex: 1 }}><Text style={[styles.cardName, { color: theme.text }]}>{item.name}</Text><Text style={[styles.cardSub, { color: theme.subText }, overdue && styles.redText]}>Nächster: {item.dates.find(d => !d.completed)?.dateString || "Erledigt"}</Text></View>
                            ...
                            {item.dates.map((d) => (
                              <View key={d.id} style={[styles.detailRow, { borderTopColor: theme.border }]}>
                                <TouchableOpacity style={styles.checkArea} onPress={() => toggleDateStatus(item.id, d.id)}>
                                  <View style={[styles.checkCircle, { borderColor: isDarkMode ? '#48484A' : '#DDD' }, d.completed && styles.checkActive, (isOverdue(d.dateObj) && !d.completed) && styles.checkOverdue]}>{d.completed && <Text style={{color: '#FFF', fontSize: 10}}>✓</Text>}</View>
          ...
          {activeTab === 'stats' && (
            <ScrollView style={styles.statsContainer} showsVerticalScrollIndicator={false}>
               ...
               <View style={styles.statsHeaderBox}><Text style={[styles.miniLabel, { color: '#28A745', marginBottom: 15 }]}>EINNAHMEN</Text></View>
               {yearlyIncomeStats.map((item, idx) => (
          ...
          {activeTab === 'backup' && (
            <ScrollView contentContainerStyle={styles.formContainer}>
              ...
              <View style={[styles.inputGroup, { backgroundColor: theme.inputBg, borderColor: theme.border, padding: 15 }]}><TouchableOpacity style={[styles.addBtn, { height: 40 }]} onPress={handleExport}><Text style={styles.addBtnText}>Daten exportieren</Text></TouchableOpacity></View>
              <View style={[styles.inputGroup, { backgroundColor: theme.inputBg, borderColor: theme.border, padding: 15, marginTop: 10 }]}><TextInput style={[styles.input, { height: 100, color: theme.text, backgroundColor: isDarkMode ? '#2C2C2E' : '#F0F0F0', borderRadius: 10, borderBottomWidth: 0 }]} placeholder="Backup-Code hier einfügen..." multiline value={importCode} onChangeText={setImportCode} placeholderTextColor={theme.subText} /><TouchableOpacity style={[styles.addBtn, { height: 40, marginTop: 15, backgroundColor: '#28A745' }]} onPress={handleImport}><Text style={styles.addBtnText}>Backup jetzt einspielen</Text></TouchableOpacity></View>
            </ScrollView>
          )}
        </KeyboardAvoidingView>
      </View>
