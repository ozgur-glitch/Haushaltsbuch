...
export default function App() {
  ...
  const [importCode, setImportCode] = useState(''); // Neu für Backup
  ...
  const handleImport = () => {
    try {
      ...
      if (parsed.claims && parsed.incomes) {
        // Wiederherstellung der Date-Objekte für die Berechnungslogik
  ...
  const pickImage = () => {
    Alert.alert("Foto hinzufügen", "Hier würde sich der Image-Picker öffnen.", [
      { text: "Beispielbild nutzen", onPress: () => setSelectedImage('https://via.placeholder.com/300x400.png?text=Kassenbon+Muster') },
  ...
  const saveEntry = () => {
    ...
    // Korrektur: Ersetzt Komma durch Punkt für die Berechnung
                      ...
                      <View key={item.id} style={[styles.card, { backgroundColor: theme.card }]}>
                        <TouchableOpacity activeOpacity={0.8} style={styles.cardMain} onPress={() => setExpandedId(isExpanded ? null : item.id)} onLongPress={() => deleteWholeClaim(item.id)}>
                          <View style={[styles.cardIconBox, { backgroundColor: isDarkMode ? '#2C2C2E' : '#F0F4FF' }, overdue && {backgroundColor: isDarkMode ? '#3D1B1B' : '#FFF0F0'}]}>
                            <Text style={{fontSize: 20}}>{overdue ? '⚠️' : (item.image ? '📄' : '📅')}</Text>
                          </View>
                          <View style={{ flex: 1 }}>
                            <Text style={[styles.cardName, { color: theme.text }]}>{item.name}</Text>
